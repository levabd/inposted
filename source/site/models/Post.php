...
class Post extends \shared\models\Post
{
    ...
    public function scopes() {
        return [
            ...
            'good'   => ['condition' => "(SELECT COUNT(`Post_id`) FROM `Vote` where `type` != 'like' AND $this->tableAlias.id = `Vote`.`Post_id`) <= (30 + (SELECT COUNT(`Post_id`) FROM `Vote` where `type` = 'like' AND $this->tableAlias.id = `Vote`.`Post_id`))"],
//            'good'   => ['condition' => "(SELECT COUNT(`Post_id`) FROM `Vote` where `type` != 'like' AND $this->tableAlias.id = `Vote`.`Post_id`) = 0"],
    ...
    public function getIsGood() {
        return $this->dbConnection->createCommand('SELECT (SELECT COUNT(*) FROM `Vote` WHERE `type` != "like" AND `Post_id` = :id) <= (30 + (SELECT COUNT(*) FROM `Vote` WHERE `type` = "like" AND `Post_id` = :id))')
        ...
        if ($content) {
            ...
            if (preg_match_all(self::REGEX_URL, $content, $matches)) {
                ...
                foreach (array_unique($matches[2]) as $url) {
                    ...
                    $htmlContent = str_replace($url, \CHtml::link('Источник', $shortUrl), $htmlContent);
                }
            }
